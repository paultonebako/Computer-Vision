# Script for keeping only the frames where the dogs are standing
...
def PredictThisImage(image, models, params):
    classes = ['Lies_on_floor', 'Sitting', 'Standing']
...
if __name__ == '__main__':
    ...
    class_names = ['Lies_on_floor', 'Sitting', 'Standing']
    ...
    if not params.debug:
        ...
        ap.add_argument("-f", "--min_number_of_frames", required=True,
                        help="Minimum number of good frames necessary to evaluate the video as good")
        args = vars(ap.parse_args())
        good_frames_threshold = int(args["min_num_of_frames"])
    ...
    for vid_name in listofvideos:
        ...
        while cap.isOpened():
            ...
            if is_dog:
                ...
                # Uses model to classify the dog pose in the frame
                label, scores = PredictThisImage(or_frame, model, params)

                # Saves the frame if the dog is standing
                if np.argmax(scores) == 2:
                    acceptable_frames += 1
                    cv2.imwrite(frames_folder+"/" + "_" + out_put_video_name +
                            "_fn"+str(frame_num)+".jpg", out_frame)
        ...
        print("Total frames with dog standing: ", acceptable_frames)
        print("Total number of frames: ", frame_num)

        # Decides whether the video is acceptable based on the number of frames
        if acceptable_frames >= good_frames_threshold:
            print("Video is acceptable")
        else:
            print("Video is not acceptable")
